...
static const char CONFIG_PATH[] = "c:\\Projet-C\\Projet-C-main\\config.ini";
...
void menu_free(Menu *m)
{
    for (int i = 0; i < m->count; i++)
        if (m->items[i]) SDL_DestroyTexture(m->items[i]);
    free(m->items);
    free(m->rects);
}
...
void load_config(int *vol, int *w, int *h)
{
    ...
    *h = DEFAULT_HEIGHT;
    ...
    while (fscanf(f, "%63[^=]=%63s\n", key, val) == 2) {
        if (strcmp(key, "volume") == 0) *vol = atoi(val);
        else if (strcmp(key, "width") == 0) *w = atoi(val);
        else if (strcmp(key, "height") == 0) *h = atoi(val);
    }
    fclose(f);
}
...
int main()
{
    ...
    Mix_Music *bgm = Mix_LoadMUS("c:\\Projet-C\\Projet-C-main\\assets\\audio\\menu_bgg.ogg");
    if (bgm) {
    ...
    TTF_Font *font = TTF_OpenFont("c:\\Projet-C\\Projet-C-main\\assets\\fonts\\Titre_menu.ttf", 64);
    SDL_Texture *bg = IMG_LoadTexture(renderer, "c:\\Projet-C\\Projet-C-main\\assets\\background\\image_fondv1.png");
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&e)) {
            if (e.type == SDL_QUIT)
                ...
                if (state == MENU_MAIN) {
                    if (s == 0) { state = MENU_JOUER; currentMenu = jouerMenu; }
                    if (s == 1) { state = MENU_PARAM; currentMenu = paramMenu; }
                    if (s == 2) running = 0;
                }

                else if (state == MENU_JOUER && s == 2) {
                    state = MENU_MAIN; currentMenu = mainMenu;
                }

                else if (state == MENU_PARAM) {
                    if (s == 0) { state = MENU_AUDIO; currentMenu = audioMenu; }
                    if (s == 1) { state = MENU_RESOL; currentMenu = resMenu; }
                    if (s == 2) { state = MENU_MAIN; currentMenu = mainMenu; }
                }
                    ...
                    if (s < 4) {
                        ...
                        state = MENU_PARAM; currentMenu = paramMenu;
                    }
                }
                ...
                if (e.key.keysym.sym == SDLK_LEFT)  vol = (vol > 0) ? vol - 5 : 0;
                if (e.key.keysym.sym == SDLK_RIGHT) vol = (vol < 100) ? vol + 5 : 100;
